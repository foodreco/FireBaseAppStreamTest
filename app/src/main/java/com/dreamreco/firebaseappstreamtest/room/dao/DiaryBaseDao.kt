...
//    suspend fun getRecent3MonthsVODByDateForSort(recentDate : Int, filter:MyDrink? = null): List<AllVODListBase>
    ...
    suspend fun getRecordDateBySuspend(): List<Int>

    // 기록 중 myDrink 가 존재하는 기록 일수만 모두 가져오는 함수
    @Query("SELECT dateForSort FROM diary_base WHERE myDrink NOT LIKE :filter ORDER BY dateForSort ASC")
    suspend fun getMyDrinkRecordedDateBySuspend(filter:MyDrink? = null): List<Int>

    // 기록 중 image 가 존재하는 uri 를 가져오는 함수
    @Query("SELECT image FROM diary_base")
    suspend fun getUriBySuspend() : List<Uri?> //filter:Uri? = null
    ...
    // 리스트 Fragment 검색 전용
    // myDrink 가 존재하는 모든 데이터를 가져오는 함수
    @Query("SELECT * FROM diary_base WHERE myDrink NOT LIKE :filter ORDER BY dateForSort ASC")
    fun getDiaryBaseWithMyDrinkByFlow(filter:MyDrink? = null): Flow<List<DiaryBase>>
