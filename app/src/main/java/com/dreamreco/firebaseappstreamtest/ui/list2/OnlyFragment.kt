...
//             정렬 설정에 따라 가져오는 데이터
            sortNumber.observe(viewLifecycleOwner) { sortType ->
                when (sortType) {
                    SORT_NORMAL -> {
                        getAllOnlyDataDESC().observe(viewLifecycleOwner) {
                    ...
                    SORT_IMPORTANCE -> {
                        getOnlyDataImportant().observe(viewLifecycleOwner) {
...
//            직접 호출로 일단 진행
    ...
    private fun setRecyclerView(typeface: Typeface) {
        mAdapter = OnlyFragmentAdapter(requireContext(), typeface, getThemeType())
        with(binding) {
