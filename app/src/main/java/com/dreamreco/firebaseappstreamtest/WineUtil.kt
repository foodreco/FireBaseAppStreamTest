...
object WineUtil {
     ...
     */
    fun getRandom(context: Context): Wine {
        val wine = Wine()
        val random = Random()
        ...
        wine.name = getRandomName(random)
        wine.country = getRandomString(cities, random)
        wine.category = getRandomString(categories, random)
        wine.photo = getRandomImageUrl(random)
        wine.price = getRandomInt(prices, random)
        wine.numRatings = random.nextInt(20)
        wine.avgRating = ThreadLocalRandom.current().nextDouble(1.0,5.0)
     ...
     */
    fun getPriceString(wine: Wine): String {
        return getPriceString(wine.price)
    }
